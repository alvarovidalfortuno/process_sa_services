...
router.post('/usuarioCreate', (req, res) => {
    ...
    async function usuarioCreate() {
        ...
        try {
            ...
            const result = await connection.execute('INSERT INTO USUARIOS VALUES(:1,:2,:3)', ['(SELECT MAX(ID_USUARIO)+1 FROM DUAL)', correo_usuario_IN, password_IN]);
            res.status(200).json({
                ok: true,
...
router.put('/usuarioUpdate', (req, res) => {
    ...
    async function usuarioUpdate() {
        ...
        try {
            ...
            const result = await connection.execute('INSERT INTO USUARIOS VALUES(:1,:2,:3)', ['(SELECT MAX(ID_USUARIO)+1 FROM DUAL)', 'usuario12@gmail.com', 'testpassword']);
            console.log(result);
            res.status(200).json({
                ok: true,
                message: 'Usuario creado exitosamente!'
